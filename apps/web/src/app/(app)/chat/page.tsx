...
import { Loader2, Send, MessageSquare, Mic, Paperclip, Pin } from 'lucide-react';
...
interface Message {
  ...
  editedAt?: string | null; pinned?: boolean;
...
export default function ChatPage() {
  ...
  const uploadVoice = async (blob: Blob, mimeType: string) => {
    ...
    const token = localStorage.getItem('accessToken');
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4020/api/v1'}/chat/rooms/${activeRoom}/upload`, {
                    ...
                    <div key={m.id} className={`flex ${isMine ? 'justify-start' : 'justify-end'}`}>
                      <div className={`max-w-[75%] rounded-lg px-3 py-2 ${isMine ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                        ...
                        <div className={`flex items-center gap-2 mt-1 ${isMine ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>
                          ...
                          {isMine && m.type !== 'SYSTEM' && !editingMsg && (
                            <span className="flex gap-1 text-[9px]">
                              <button className="underline opacity-70" onClick={() => { setEditingMsg(m.id); setEditText(m.content); }}>ویرایش</button>
                              <button className="underline opacity-70" onClick={() => deleteMessage.mutate(m.id)}>حذف</button>
                            </span>
                          )}
                          {m.type !== 'SYSTEM' && (
                            <button className={`text-[9px] underline ${m.pinned ? 'opacity-100' : 'opacity-50'}`} onClick={() => pinMessage.mutate({ msgId: m.id, pinned: !m.pinned })}>
                              {m.pinned ? '📌 سنجاق شده' : 'سنجاق'}
                            </button>
                          )}
                        </div>
                      </div>
